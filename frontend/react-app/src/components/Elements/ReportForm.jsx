...
const GenerateReportForm = () => {
  ...
  const processFiles = async (files) => {
    ...
    try {
      ...
      fileDetails.forEach((detail) => {
        const formattedStartDate = detail.startDate
          ? new Date(detail.startDate)
              .toLocaleDateString("en-GB")
              .replace(/\//g, "-")
          : "";
        const formattedEndDate = detail.endDate
          ? new Date(detail.endDate)
              .toLocaleDateString("en-GB")
              .replace(/\//g, "-")
          : "";

        payload.bank_names.push(detail.bankName || "");
        payload.passwords.push(detail.password || "");
        payload.start_date.push(formattedStartDate);
        payload.end_date.push(formattedEndDate);
      });

      console.log("Payload:", payload);

      // In a real implementation, you would first upload files and get their paths
      files.forEach((file) => {
        // payload.pdf_paths.push(`uploads/${file.name}`);
      });
      ...
      const jsonResults = JSON.stringify(response.data, null, 2);
      const blob = new Blob([jsonResults], { type: "application/json" });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `analysis_${caseId}.json`;
